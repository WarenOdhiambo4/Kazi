...
// All sale/purchase/transfer/adjustment stock movements MUST go through these.
// The frontend (src/lib/inventoryMovements.ts) calls these instead of writing
// InventoryStock / InventoryLedger directly from React.
// ============================================================================

const handleInventoryError = (res: express.Response, error: unknown) => {
  if (error instanceof InsufficientStockError) {
    return res.status(409).json({ error: 'Insufficient stock', details: error.message });
  }
  if (error instanceof LockAcquisitionError) {
    return res.status(423).json({ error: 'Stock is busy, please retry', details: error.message });
  }
  // eslint-disable-next-line no-console
...
app.post('/inventory/sales/deduct', async (req, res) => {
  try {
    const { businessUnitId, productId, quantity, unitCost, referenceType, referenceId, authorisedByUserId, createdByUserId } =
      req.body as Record<string, any>;
    if (!businessUnitId || !productId || !quantity) {
      return res.status(400).json({ error: 'businessUnitId, productId, and quantity are required' });
    }
    const result = await deductStock({
      ...
      referenceType: referenceType ?? 'SalesTransactionItem',
...
app.post('/inventory/purchases/receive', async (req, res) => {
  try {
    const { businessUnitId, productId, quantity, unitCost, referenceType, referenceId, authorisedByUserId, createdByUserId } =
      req.body as Record<string, any>;
    if (!businessUnitId || !productId || !quantity) {
    ...
    const result = await addStock({
      ...
      unitCost: unitCost !== undefined ? Number(unitCost) : undefined,
      referenceType: referenceType ?? 'PurchaseOrderDeliveryItem',
      referenceId: referenceId !== undefined ? Number(referenceId) : undefined,
      authorisedByUserId,
      createdByUserId,
    });
...
app.post('/inventory/transfers', async (req, res) => {
  try {
    const { fromBusinessUnitId, toBusinessUnitId, productId, quantity, unitCost, authorisedByUserId, createdByUserId } =
      req.body as Record<string, any>;
    if (!fromBusinessUnitId || !toBusinessUnitId || !productId || !quantity) {
      return res.status(400).json({
        error: 'fromBusinessUnitId, toBusinessUnitId, productId, and quantity are required',
      });
    }
...
app.post('/inventory/adjustments', async (req, res) => {
  try {
    const { businessUnitId, productId, newQuantity, quantityChange, unitCost, authorisedByUserId, createdByUserId } = req.body as Record<string, any>;
    if (!businessUnitId || !productId || (newQuantity === undefined && quantityChange === undefined)) {
      return res
        .status(400)
        .json({ error: 'businessUnitId, productId, and either newQuantity or quantityChange are required' });
    }
    const result = await adjustStock({
      businessUnitId,
      productId,
      newQuantity: newQuantity !== undefined ? Number(newQuantity) : undefined,
      quantityChange: quantityChange !== undefined ? Number(quantityChange) : undefined,
      unitCost: unitCost !== undefined ? Number(unitCost) : undefined,
      authorisedByUserId,
      createdByUserId,
    });
...
app.get('/inventory/stock/:businessUnitId/:productId', async (req, res) => {
  try {
    const stock = await getStock(req.params.businessUnitId, req.params.productId);
...
app.get('/inventory/ledger/:businessUnitId/:productId', async (req, res) => {
  try {
    const maxRecords = req.query.maxRecords ? Number(req.query.maxRecords) : 50;
    const records = await getLedgerHistory(req.params.businessUnitId, req.params.productId, maxRecords);
...
app.get('/inventory/reconcile/:businessUnitId/:productId', async (req, res) => {
  try {
    const report = await reconcileStock(req.params.businessUnitId, req.params.productId);
...
// // INVENTORY SERVICE ENDPOINTS
// // All sale/purchase/transfer/adjustment stock movements MUST go through these.
// // n8n (and anything else) should call these instead of writing InventoryStock /
// // InventoryLedger directly.
// // ============================================================================
...
// app.post('/inventory/adjustments', async (req, res) => {
//   try {
//     const { businessUnitId, productId, newQuantity, authorisedByUserId, createdByUserId } = req.body as Record<string, any>;
//     if (!businessUnitId || !productId || newQuantity === undefined) {
//       return res.status(400).json({ error: 'businessUnitId, productId, and newQuantity are required' });
//     }
//     const result = await adjustStock({
//       businessUnitId,
//       productId,
...
// // Diagnostic endpoint: replays the full ledger history for a product/branch and reports
// // whether it's internally consistent and matches the current stored stock balance.
// // Use this to audit existing data (e.g. productId 147) for the lost-update pattern.
// app.get('/inventory/reconcile/:businessUnitId/:productId', async (req, res) => {
//   try {
//     const report = await reconcileStock(req.params.businessUnitId, req.params.productId);
//     return res.json(report);
