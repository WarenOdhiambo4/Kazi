...
export class InsufficientStockError extends Error {
  constructor(productId: string, businessUnitId: string, available: number, requested: number) {
    super(
      `Insufficient stock for product ${productId} at business unit ${businessUnitId}: ` +
  ...
  return withInventoryLock(params.businessUnitId, params.productId, async () => {
    ...
    if (!stockRecord) {
      if (params.quantityChange < 0 && !params.allowNegative) {
        throw new InsufficientStockError(params.productId, params.businessUnitId, 0, -params.quantityChange);
      }
